...
#tag WindowCode
		...
		Sub prefix(letters as string, lplay as integer, word as string, node as integer, x as integer, y as integer, horizontal as boolean, offset as integer, pvalue as integer, pmult as integer, psum as integer)
		  ...
		  if dagadag_endword(node) then
		    if unique_play(lplay,x,y,horizontal) then
		      process(if(horizontal,str(x)+chr(y+offset+65),chr(y+64)+str(x+offset+1))+"-"+if(horizontal,h_rc(x,y),v_cr(x,y)),word,psum+pvalue*pmult+if(lplay=7,50,0),letters)
		    end
		  end
		  rack.import letters
	...
	#tag Method, Flags = &h0
		Sub suffix(letters as string, lplay as integer, word as string, node as integer, x as integer, y as integer, horizontal as boolean, offset as integer, lmost as integer, pvalue as integer, pmult as integer, psum as integer)
		  ...
		  if dagadag_endword(node) then
		    if unique_play(lplay,x,y,horizontal) then
		      process(if(horizontal,str(x)+chr(y+lmost+65),chr(y+64)+str(x+lmost+1))+"-"+if(horizontal,h_rc(x,y),v_cr(x,y)),word,psum+pvalue*pmult+if(lplay=7,50,0),letters)
		    end
		  end
		  rack.import letters
		  if if(horizontal,board(x+offset,y).face,board(x,y+offset).face)="" then
